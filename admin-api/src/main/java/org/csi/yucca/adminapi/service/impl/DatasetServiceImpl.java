...
public class DatasetServiceImpl implements DatasetService {
	...
	public ServiceResponse insertCSVData(MultipartFile file, Boolean skipFirstRow, String encoding, String csvSeparator, String componentInfoRequestsJson, String organizationCode,
			Integer idDataset, JwtUser authorizedUser) throws BadRequestException, NotFoundException, Exception {
	...
	private void updateDatasetTransaction(DatasetRequest datasetRequest) throws Exception {
		...
		if (!datasetRequest.getUnpublished()) {
			...
			try {
				...
				apiName = PublisherDelegate.build().addApi(httpclient, update, datasetRequest);
				...
				if (duplicateException.getMessage() != null && duplicateException.getMessage().toLowerCase().contains("duplicate")) {
					try {
						...
						update = true;
						PublisherDelegate.build().addApi(httpclient, update, datasetRequest);
					} catch (Exception e) {
	...
	public ServiceResponse insertDataset(String organizationCode, DatasetRequest postDatasetRequest, JwtUser authorizedUser) throws BadRequestException, NotFoundException,
		...
		return ServiceResponse.build().object(PostDatasetResponse.build(dataset.getIddataset()).datasetcode(dataset.getDatasetcode()).datasetname(dataset.getDatasetname()));
	}
	 ...
	 * @throws Exception
	 */
	private Dataset insertDatasetTransaction(DatasetRequest postDatasetRequest, JwtUser authorizedUser, Organization organization) throws Exception {
		...
		// API
		logger.info("[DatasetServiceImpl::insertDatasetTransaction] Publish API - postDatasetRequest.unpublished: " + postDatasetRequest.getUnpublished());
		if (!postDatasetRequest.getUnpublished()) {
			...
			try {
				logger.info("[DatasetServiceImpl::insertDatasetTransaction] Publish API - add");
				PublisherDelegate.build().addApi(httpclient, update, postDatasetRequest);
			} catch (Exception duplicateException) {
				logger.info("[DatasetServiceImpl::insertDatasetTransaction] Publish API - ERROR " + duplicateException.getMessage());
				if (duplicateException.getMessage() != null && duplicateException.getMessage().toLowerCase().contains("duplicate")) {
					try {
