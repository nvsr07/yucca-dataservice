...
public class PublisherDelegate {
	...
	public String addApi(CloseableHttpClient httpclient, boolean update, /*IDataSourceRequest*/ DettaglioDataset dataset) throws HttpException, IOException {
		...
		String apiFinalName = dataset.getDatasetcode() + "_odata";
		...
		if (update)
			addApiParams.add(new BasicNameValuePair("actionAPI","updateAPI"));
		else
			addApiParams.add(new BasicNameValuePair("actionAPI","addAPI"));
...
//		logger.debug("[PublisherDelegate::generetateKey] application: " + application);
...
//		String url = publisherUrl + "site/blocks/subscription/subscription-add/ajax/subscription-add.jag";
...
//	}
	...
	public void publishApi(CloseableHttpClient httpclient, String apiVersion, String apiName, String provider) throws HttpException, IOException {
...
//		logger.debug("[PublisherDelegate::createApiForBulk] appName: " + appName);
...
//		} else {
//			addApplicationParams.add(new BasicNameValuePair("visibility", "restricted");
//
//			String ruoli = "";
//
//			if (dataset.getTenantssharing() != null && dataset.getTenantssharing().getTenantsharing() != null) {
//				for (org.csi.yucca.storage.datamanagementapi.model.metadata.Tenantsharing t : dataset.getTenantssharing().getTenantsharing()) {
//					if (!ruoli.equals(""))
//						ruoli += ",";
...
//			addApplicationParams.add(new BasicNameValuePair("authType", "Application & Application User");
//		}
//
//		if (update) {
//			addApplicationParams.add(new BasicNameValuePair("actionAPI", "updateAPI");
//		} else {
//			addApplicationParams.add(new BasicNameValuePair("actionAPI", "addAPI");
//		}
//
//		addApplicationParams.add(new BasicNameValuePair("apimanConsoleAddress", Config.getInstance().getConsoleAddress());
//		addApplicationParams.add(new BasicNameValuePair("username", Config.getInstance().getStoreUsername());
//		addApplicationParams.add(new BasicNameValuePair("password", Config.getInstance().getStorePassword());
//		addApplicationParams.add(new BasicNameValuePair("httpok", Config.getInstance().getHttpOk());
//		addApplicationParams.add(new BasicNameValuePair("ok", Config.getInstance().getResponseOk());
//
//		// addApplicationParams.add(new BasicNameValuePair("icon", path + fileName);
...
//		addApplicationParams.add(new BasicNameValuePair("context", "/api/" + apiName);// ds_Voc_28;
//		addApplicationParams.add(new BasicNameValuePair("P", "");
//		addApplicationParams.add(new BasicNameValuePair("endpoint", Config.getInstance().getBaseApiUrl() + apiName);
//		addApplicationParams.add(new BasicNameValuePair("desc", dataset.getDescription() != null ? Util.safeSubstring(dataset.getDescription(), API_FIELD_MAX_LENGTH) : "");
//		addApplicationParams.add(new BasicNameValuePair("copiright", dataset.getCopyright() != null ? Util.safeSubstring(dataset.getCopyright(), API_FIELD_MAX_LENGTH) : "");
//
//		addApplicationParams.add(new BasicNameValuePair("extra_isApi", "false");
//		addApplicationParams.add(new BasicNameValuePair("extra_apiDescription", dataset.getDatasetName() != null ? dataset.getDatasetName() : "");
//		addApplicationParams.add(new BasicNameValuePair("codiceTenant", dataset.getTenantCode() != null ? dataset.getTenantCode() : "");
//		addApplicationParams.add(new BasicNameValuePair("codiceStream", "");
//		addApplicationParams.add(new BasicNameValuePair("nomeStream", "");
//		addApplicationParams.add(new BasicNameValuePair("nomeTenant", dataset.getTenantCode() != null ? dataset.getTenantCode() : "");
//		addApplicationParams.add(new BasicNameValuePair("licence", dataset.getLicense() != null ? Util.safeSubstring(dataset.getLicense(), API_FIELD_MAX_LENGTH) : "");
//		addApplicationParams.add(new BasicNameValuePair("disclaimer", dataset.getDisclaimer() != null ? Util.safeSubstring(dataset.getDisclaimer(), API_FIELD_MAX_LENGTH) : "");
//		addApplicationParams.add(new BasicNameValuePair("virtualEntityName", "");
//		addApplicationParams.add(new BasicNameValuePair("virtualEntityDescription", "");
//
//		String tags = "";
//
...
//		if (dataset.getTags() != null) {
//			tagCodes = new LinkedList<String>();
//			for (org.csi.yucca.storage.datamanagementapi.model.metadata.Tag t : dataset.getTags()) {
//				tags += "," + t.getTagCode();
//				tagCodes.add(t.getTagCode());
//			}
//		}
//
//		addApplicationParams.add(new BasicNameValuePair("tags", Util.safeSubstring(tags, API_FIELD_MAX_LENGTH));
//
//		// DT Add document ? Why restart from jsonFile? we lost init
//		//String contentJson = extractMetadataContentForDocument(jsonFile,dataset.getTenantCode() != null ? dataset.getTenantCode() : "");
//		String contentJson = extractMetadataContentForDocument(metadata,dataset.getTenantCode() != null ? dataset.getTenantCode() : "");
//		
//		
//		//SOLR
//		//addApplicationParams.add(new BasicNameValuePair("content", contentJson);
