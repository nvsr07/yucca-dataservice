package it.csi.smartdata.dataapi.multiapi.util;
...
public class ReplacingInputStream extends FilterInputStream {
	 ...
	 */
	public ReplacingInputStream(InputStream in, String pattern,
			String replacement) {
		this(in, pattern.getBytes(UTF8), replacement == null ? null
				: replacement.getBytes(UTF8));
	}
	 ...
	 *            the replacement or null
	 */
	public ReplacingInputStream(InputStream in, byte[] pattern,
			byte[] replacement) {
		super(in);
	...
	public int read(byte[] b, int off, int len) throws IOException {
		// copy of parent logic; we need to call our own read() instead of
		// super.read(), which delegates instead of calling our read
		if (b == null) {
	...
	public int read() throws IOException {
		...
		case NOT_MATCHED:
			...
			if (pattern.length == 1) {
				// edge-case when the pattern length is 1 we go straight to
				// replacing
		...
		case REPLACING:
			// we've fully matched the pattern and are returning bytes from the
			// replacement
		...
		case UNBUFFER:
			// we partially matched the pattern before encountering a non
			// matching byte
			// we need to serve up the buffered bytes before we go back to
			// NOT_MATCHED
	...
	public String toString() {
		return state.name() + " " + matchedIndex + " " + replacedIndex + " "
				+ unbufferIndex;
	}
